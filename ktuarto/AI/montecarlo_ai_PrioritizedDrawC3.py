...
class MontecarloPrioritizedDrawC3(base_ai.BaseAi):
    """
    ゲーム木を生成するクラス
    内容は適当な値を返す処理を記述
    callだけは判定する
...
class MonteBranchInfo:
    ...
    def playout(self):
        ...
        #枝の場合。ブランチがある場合、さらにその配下のplayoutを実行し、その結果を返す。
            ...
            if(np.sum(self.branchPlayoutWin) > np.sum(self.branchPlayoutCount) - np.sum(self.branchPlayoutDraw) - np.sum(self.branchPlayoutWin)): #勝率が少しでもあるならそれを使う。
                u = ucb1.UCB1.ucb1(self.branchPlayoutCount, self.branchPlayoutWin - (self.branchPlayoutCount - self.branchPlayoutDraw - self.branchPlayoutWin), self.branchPlayoutTotal, self.param.ucb1_c, self.param.ucb1_fpu)
